...
function arrayContiene(array, elemento) {
  ...
  // Tu código:
  return array.reduce(elemento) ? true : false;
}
...
function promedioResultadosTest(resultadosTest) {
  ...
  const resultado2 = resultado / resultadosTest.length;
  return resultado2;
}
...
function numeroMasGrande(numeros) {
  ...
  // Tu código:
  const mayor = Math.max.apply(Math, numeros);
  return mayor;
}
...
function mesesDelAño(array) {
  ...
  const resultado = array.filter((mes) => {
    if (mes === "Enero" || mes === "Marzo" || mes === "Nobiembre") {
      return mes;
    }
  });
